...
def myapp(environ, start_response):
    ...
    tree= unquote(split_l[1].split("act=")[1],'utf-8')
    ##tree1:gene phylotree tree2:hap phylotree 
    tree_newick=''
    if tree == "tree1":
        with open("/var/www/hap.bioinf.club/webapp//gene_data/%s.newick"%(gene),'r') as load_f:
            tree_newick = load_f.read()
    else:
        with open("/var/www/hap.bioinf.club/webapp//gene_data/%s.hap.newick"%(gene),'r') as load_f:
            tree_newick = load_f.read()
    
    url = 'https://itol.embl.de/upload.cgi'
    data = {'tname': gene, 'ttext': tree_newick}
